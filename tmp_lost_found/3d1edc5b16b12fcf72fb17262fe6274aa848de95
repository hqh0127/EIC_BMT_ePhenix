...
void plot_matscan_ave(const char *datfile, const int plot_theta = 0)
{
  FILE *f = fopen(datfile,"r");
  if (!f)
    {
  ...
  if (lastdot != std::string::npos){
	...
	TFile* fout = new TFile("materialscan.root","update");
	h1->Write(filename.c_str(),TObject::kOverwrite);
	fout->Close();
